...
from gmo.auth import GMOCoinAuth
...
class GMOCoin(object):
    ...
    def klines(self, symbol, interval, date):
        """
        取引履歴
        指定した銘柄の取引履歴を取得します。
        :param symbol: *required BTC ETH BCH LTC XRP BTC_JPY ETH_JPY BCH_JPY LTC_JPY XRP_JPY
        ...
        :type interval: str
        :param date: *required 指定可能な日付フォーマット
        :type date: str
        """
        payload = {
            'symbol': symbol, 
        ...
        return self._request('GET', '/v1/klines', payload, auth=False)
    ...
    def openpositions(self, symbol, page=1, count=100):
        """
        建玉一覧を取得
        有効建玉一覧を取得します。
        対象: レバレッジ取引
    ...
    def cancelBulkOrder(self, symbol, desc=False):
        """
        注文の一括キャンセル
        一括で注文を取消します
        対象: 現物取引、レバレッジ取引
